...
def create_app():
    ...
    eureka_client.init(eureka_server=f'http://{eureka_server_host}:8761/eureka/', app_name="postal-service", instance_port=int(app.config.get("FLASK_RUN_PORT")), instance_host=instance_host)
    CORS(app)
    route_index(app)
    return app
